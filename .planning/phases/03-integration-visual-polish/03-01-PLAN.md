...
- Pass home data and userEmail to children via React context or by restructuring data flow through the sub-pages
...
Important: The layout needs to pass `home` and `userEmail` data down to sub-pages. Since these are server components using App Router, the cleanest approach is to have each sub-page fetch its own data (like settings pages do) OR use a shared server-side data fetching pattern. The simplest approach: move getHomeInfo to a shared utility in `@/lib/home-api.ts` and have each sub-page call it, OR keep the home data fetch in layout and use React's cache() to avoid duplicate fetches. Choose the approach that avoids prop drilling through layouts — the recommended pattern is to create a shared `getHomePageData()` function that uses React's `cache()` from 'react' so multiple components can call it without duplicate fetches.
  </action>
  <verify>
...
Each sub-page needs a client wrapper component to handle the interactive state (useState for home data updates, action handlers). Create a single shared client component or individual ones per page. The simplest approach: create thin client wrappers that take initial data as props and handle mutations with router.refresh().

After creating sub-pages, the old monolithic `HomeSettings` component in `home-settings.tsx` can be simplified or deprecated. However, do NOT delete it yet — just stop importing it from the hemmet pages. The `HomeSettingsClient` in `home-settings-client.tsx` may also need updates to work with the new routing.
