...
import { AdminRestructureClient } from './admin-restructure-client'
import { env } from '@/lib/env'

export const metadata: Metadata = {
  title: 'Strukturera | Admin',
}
...
async function getData(page: number, search: string, mode: string): Promise<PaginatedResponse> {
...
export default async function AdminRestructurePage({ searchParams }: PageProps) {
  const params = await searchParams
  const page = Math.max(1, parseInt(params.page || '1', 10))
  const search = params.search || ''
  const mode = params.mode || 'ingredients'

  const initialData = await getData(page, search, mode)
