...
export default function ApiDokumentationPage() {
      ...
      <article>
        ...
        <div className="space-y-10 text-foreground/80 leading-relaxed">
          ...
          <section>
            ...
            <p className="mt-3">
              Alla RPC-anrop använder <Code>POST /rpc/&lt;funktionsnamn&gt;</Code>{" "}
              med JSON-kropp. Tabell-/vyfrågor använder{" "}
              <Code>GET /&lt;vynamn&gt;</Code>.
            </p>
          </section>
  ...
  -d '{
    ...
    "p_cuisine": "Italienskt"
                ...
                <div className="mt-3">
                  <ParamTable
                    params={[
                      { name: "p_name", type: "text", required: "ja", description: "Receptnamn" },
                      { name: "p_author", type: "text", required: "ja", description: "Författare" },
                      { name: "p_url", type: "text", required: "ja", description: "Käll-URL (tom sträng om ingen)" },
                      { name: "p_recipe_yield", type: "integer", required: "ja", description: "Antal portioner" },
                      { name: "p_recipe_yield_name", type: "text", required: "ja", description: 'Portionsenhet (t.ex. "portioner")' },
                      { name: "p_prep_time", type: "integer", required: "ja", description: "Förberedelsetid i minuter" },
                      { name: "p_cook_time", type: "integer", required: "ja", description: "Tillagningstid i minuter" },
                      { name: "p_description", type: "text", required: "ja", description: "Kort beskrivning" },
                      { name: "p_categories", type: "text[]", required: "ja", description: "Kategorinamn" },
                      { name: "p_ingredients", type: "jsonb[]", required: "ja", description: "Ingrediensobjekt (se datamodell)" },
                      { name: "p_instructions", type: "jsonb[]", required: "ja", description: "Instruktionsobjekt (se datamodell)" },
                      { name: "p_cuisine", type: "text", required: "nej", description: "Typ av kök" },
                      { name: "p_image", type: "text", required: "nej", description: "Bild-ID från /upload (UUID)" },
                      { name: "p_thumbnail", type: "text", required: "nej", description: "Miniatyr-ID från /upload (UUID, kan vara samma som p_image)" },
                    ]}
                  />
                </div>
              </Endpoint>
              ...
              <Endpoint name="update_recipe">
                <p>
                  ...
                  <Code>insert_recipe</Code> plus <Code>p_recipe_id</Code> (uuid,
                  krävs) och valfritt <Code>p_date_published</Code> (ISO 8601).
                  Du måste äga receptet. Returnerar inget.
                </p>
              </Endpoint>
  ...
  -d '{"p_query": "pasta", "p_owner_only": true, "p_limit": 20}'`}</CodeBlock>
                <div className="mt-3">
                  <ParamTable
                    params={[
                      { name: "p_query", type: "text", required: "ja", description: "Sökfråga" },
                      { name: "p_owner_only", type: "boolean", required: "nej", description: "Bara egna recept (standard: false)" },
                      { name: "p_category", type: "text", required: "nej", description: "Filtrera på kategori" },
                      { name: "p_limit", type: "integer", required: "nej", description: "Max resultat (standard: 50)" },
                      { name: "p_offset", type: "integer", required: "nej", description: "Sidnumrering (standard: 0)" },
                      { name: "p_owner_ids", type: "uuid[]", required: "nej", description: "Filtrera på specifika ägare" },
                    ]}
                  />
                </div>
              </Endpoint>
  ...
  -F "file=@min-bild.jpg"`}</CodeBlock>
              ...
              <Endpoint name="GET /images/:id/:size">
                ...
                <div className="mt-3">
                  <ParamTable
                    params={[
                      { name: "thumb", type: "320x240", required: "", description: "Miniatyrbild för listor och kort" },
                      { name: "small", type: "640x480", required: "", description: "Liten bild för mobilvyer" },
                      { name: "medium", type: "960x720", required: "", description: "Mellanstor bild för receptsidor" },
                      { name: "large", type: "1280x960", required: "", description: "Stor bild för desktop" },
                      { name: "full", type: "1920x1440", required: "standard", description: "Originalbild i full upplösning" },
                    ]}
                  />
                </div>
                <p className="mt-2 text-sm text-foreground/60">
...
curl -s https://api.matrummet.se/rpc/insert_recipe \\
  ...
  -d '{"p_recipe_id": "uuid-här", "p_servings": 4}'`}</CodeBlock>
                <p className="mt-2 text-sm text-foreground/60">
                  Valfritt: <Code>p_shopping_list_id</Code> (null = standardlista),{" "}
                  <Code>p_ingredient_ids</Code> (uuid[], null = alla).
                </p>
              </Endpoint>
  ...
  -d '{"p_name": "Hushållspapper", "p_shopping_list_id": "uuid-här"}'`}</CodeBlock>
                <p className="mt-2 text-sm text-foreground/60">
                  Valfritt: <Code>p_food_id</Code> (uuid) för att koppla till ett
                  livsmedel.
                </p>
              </Endpoint>
