 ...
 * Cron/non-SSE fallback — starts processing in the background via after().
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (pendingFoods.length === 0) {
      ...
      return NextResponse.json({ runId: run.id, status: 'pending_approval' })
    }

    after(() => processInBackground(run.id, pendingFoods, client, token))
