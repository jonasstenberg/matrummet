...
/**
 * Call Mistral AI to normalize food names
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[AI Review] Fetched ${allPendingFoods.length} pending foods for user ${reviewerEmail}`)
        ...
        for (let i = 0; i < batch.length; i++) {
          ...
          if (existingFood) {
            ...
            if (norm.normalizedName.toLowerCase() === food.name.toLowerCase()) {
              const approved = await approvePendingFood(token, food.id, reviewerEmail)
              if (approved) {
                summary.created++
                summary.details.push({
                  foodName: food.name,
