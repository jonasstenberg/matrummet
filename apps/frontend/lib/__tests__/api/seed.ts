 ...
 *
 * Note: The get_or_create_food RPC returns a UUID directly, not an object.
 */
...
export async function refreshRecipeIngredientSummary(
  ...
  const result = await client.rpc("refresh_recipe_ingredient_summary", {});
  if (result.error) {
