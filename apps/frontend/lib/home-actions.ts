...
export async function createHome(
  ...
  try {
    ...
    if (!token) {
      return { error: 'Du måste vara inloggad för att skapa ett hem' }
    }
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (errorJson.message === 'user-already-has-home') {
          return { error: 'Du är redan medlem i ett hem' }
        }
      } catch {
        // If we can't parse the error, fall through to generic message
      }

      return { error: 'Kunde inte skapa hemmet. Försök igen.' }
    }
...
export async function updateHomeName(
  ...
  try {
    ...
    if (!token) {
      return { error: 'Du måste vara inloggad för att uppdatera hemmet' }
    }
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (errorJson.message === 'user-has-no-home') {
          return { error: 'Du är inte medlem i något hem' }
        }
      } catch {
        // Fall through to generic message
      }

      return { error: 'Kunde inte uppdatera hemnamnet. Försök igen.' }
    }
...
export async function leaveHome(): Promise<{ success: true } | { error: string }> {
  try {
    ...
    if (!token) {
      return { error: 'Du måste vara inloggad för att lämna hemmet' }
    }
    ...
    if (!response.ok) {
      ...
      return { error: 'Kunde inte lämna hemmet. Försök igen.' }
    }
...
export async function joinHomeByCode(
  ...
  try {
    ...
    if (!token) {
      return { error: 'Du måste vara inloggad för att gå med i ett hem' }
    }
    ...
    if (!response.ok) {
      ...
      return { error: 'Kunde inte gå med i hemmet. Försök igen.' }
    }
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (errorJson.message === 'user-already-member') {
          return { error: 'Personen är redan medlem i ditt hem' }
        }
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (errorJson.message === 'member-not-found') {
          return { error: 'Användaren hittades inte i ditt hem' }
        }
        if (errorJson.message === 'cannot-remove-self') {
          return { error: 'Du kan inte ta bort dig själv. Använd "Lämna hemmet" istället' }
        }
      } catch {
        // Fall through to generic message
      }
