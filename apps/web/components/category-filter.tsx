...
export function CategoryFilter({
  ...
  const searchParams = useSearch({ strict: false }) as Record<string, string | undefined> & { categories?: string }
  ...
  const toggleCategory = useCallback(
    (category: string) => {
      const current = searchParams.categories?.split(',').filter(Boolean) ?? []
      const updated = current.includes(category)
        ? current.filter((c) => c !== category)
        : [...current, category]

      router.history.replace(buildSearchUrl(pathname, {
        ...searchParams,
  ...
  const clearAll = useCallback(() => {
    router.history.replace(buildSearchUrl(pathname, { ...searchParams, categories: undefined }))
  }, [searchParams, router, pathname])
