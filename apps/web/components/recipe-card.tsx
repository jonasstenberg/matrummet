...
import { Clock, Heart, Users, UtensilsCrossed } from "@/lib/icons";
...
function PlaceholderImage() {
  return (
    <div className="absolute inset-0 bg-linear-to-br from-muted via-muted/80 to-muted/60">
      ...
      <div className="absolute inset-0 bg-dots-sm opacity-30" />
      {/* Centered icon */}
...
function getMatchColor(percentage: number): string {
  if (percentage >= 80) return "text-(--color-success)";
  if (percentage >= 60) return "text-(--color-warning)";
  return "text-(--color-caution)";
}

function getMatchBgColor(percentage: number): string {
  if (percentage >= 80) return "bg-(--color-success)";
  if (percentage >= 60) return "bg-(--color-warning)";
  return "bg-(--color-caution)";
}

export function RecipeCard({ recipe, className, showPantryMatch, showAuthor, onAuthorClick }: RecipeCardProps) {
  ...
  return (
    <Link to="/recept/$id" params={{ id: recipe.id }} preloadDelay={200} className="block">
      <article
        className={cn(
      ...
      >
        ...
        <div className="p-4">
          ...
          {/* Author attribution */}
          {showAuthor && recipe.owner_name && recipe.owner_id && !recipe.is_owner && (
            <div className={cn("pt-2", (totalTime || recipe.recipe_yield) ? "mt-2 border-t border-border/50" : "mt-3")}>
              <button
                type="button"
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  onAuthorClick?.(recipe.owner_id!);
                }}
                className="text-sm text-muted-foreground hover:text-primary transition-colors"
              >
                av {recipe.owner_name}
              </button>
            </div>
          )}
        </div>
