...
export function SearchBar({ className }: SearchBarProps) {
  ...
  const searchParams = useSearch({ strict: false }) as { q?: string; members?: string }
  ...
  function getSearchNavigation(term: string) {
    const base = isAllRecipesPage ? '/alla-recept/sok' : '/sok'
    const search: Record<string, string> = { q: term }
    // Preserve member filter across searches
    if (searchParams.members) search.members = searchParams.members
    return { to: base, search }
  }

  function getEmptyNavigation() {
    const base = isAllRecipesPage ? '/alla-recept' : '/'
    const search: Record<string, string> = {}
    // Preserve member filter when clearing search
    if (searchParams.members) search.members = searchParams.members
    return { to: base, search: Object.keys(search).length > 0 ? search : undefined }
  }
  ...
  function handleSearch(term: string) {
    ...
    debounceTimerRef.current = setTimeout(() => {
      startTransition(() => {
        if (term.trim()) {
          router.navigate(getSearchNavigation(term.trim()))
        } else {
          // Empty search - go to appropriate page
          router.navigate(getEmptyNavigation())
        }
      })
    }, 300)
  }
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(() => {
      if (term) {
        router.navigate(getSearchNavigation(term))
      } else {
        router.navigate(getEmptyNavigation())
      }
    })
