...
# Flyway Migration Runner for Matrummet Database
...
create_backup() {
    ...
    local backup_file="$BACKUP_DIR/matrummet_${reason}_${timestamp}.sql"
...
restore_backup() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        psql -d "$DB_NAME" < "$backup_file" > /dev/null 2>&1
...
run_seed() {
    ...
    psql matrummet -c "ALTER TABLE user_passwords DISABLE TRIGGER encrypt_password;"

    echo "  Loading data from $SEED_FILE"
    psql matrummet -f "$SEED_FILE"

    echo "  Re-enabling password encryption trigger"
    psql matrummet -c "ALTER TABLE user_passwords ENABLE TRIGGER encrypt_password;"
