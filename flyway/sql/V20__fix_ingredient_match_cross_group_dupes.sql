-- Fix: Revert PARTITION BY to (instruction_id, ingredient_name) only.
-- V19 added quantity+measurement to the partition, which stopped dedup of
-- same-named ingredients from different groups with different quantities.
-- E.g. "2 dl vatten" (Sås) and "5 dl vatten" (Pasta) both matched any step
-- mentioning "vatten", showing duplicate badges. Now we partition by name
-- only, and the ORDER BY still prefers the same-group match.
...
-- Recompute all recipes with the corrected matching
DO $$
DECLARE
  v_recipe_id UUID;
BEGIN
