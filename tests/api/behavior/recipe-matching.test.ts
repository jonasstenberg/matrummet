...
describe("Recipe Matching Behavior", () => {
  ...
  beforeEach(async () => {
    ...
    // Clean up ALL recipes using RPC to bypass potential issues with builder pattern
    // First, delete all recipes owned by test-user-a
    await clientA.rpc("delete_all_user_recipes");
    // Then, delete all recipes owned by test-user-b
    await clientB.rpc("delete_all_user_recipes");

    // Refresh the materialized view after cleanup to remove stale data
    await clientA.rpc("refresh_recipe_ingredient_summary");
  });
  ...
  async function createRecipeWithFoods(
    ...
    const refreshResult = await clientA.rpc("refresh_recipe_ingredient_summary");
    if (refreshResult.error) {
      console.error("Failed to refresh view:", refreshResult.error);
    }
