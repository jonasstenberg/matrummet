...
describe("V49 Security: Anon Function Access Revocation", () => {
  ...
  // ==========================================================================
  describe("Admin functions reject non-admin authenticated users", () => {
    it("authenticated non-admin can call admin_list_users but gets admin check error", async () => {
      const result = await authClient.rpc("admin_list_users");

      expect(result.error).not.toBeNull();
      // The function is callable (no "permission denied"), but is_admin()
      // inside the function rejects the non-admin user
