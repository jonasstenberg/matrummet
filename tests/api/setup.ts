...
export const TEST_CONFIG = {
  ...
  JWT_SECRET:
    ...
    "matrummet-jwt-secret-change-in-production-min-32-chars",
  DB_HOST: process.env.TEST_DB_HOST || "localhost",
  DB_PORT: process.env.TEST_DB_PORT || "5433",
  DB_NAME: process.env.TEST_DB_NAME || "matrummet",
  DB_USER: process.env.TEST_DB_USER || "matrummet",
  DB_PASSWORD: process.env.TEST_DB_PASSWORD || "matrummet",
};
...
export async function signPostgrestToken(
  email: string,
  role: "authenticated" | "admin" | "anon" = "authenticated"
): Promise<string> {
...
/**
 * Create an admin client for a specific user email.
 * Uses the 'admin' DB role which inherits from 'authenticated'
 * and has access to admin-only tables and functions.
 */
export async function createAdminClient(
  email: string
): Promise<PostgrestClient> {
  const token = await signPostgrestToken(email, "admin");
  return createClient({ jwt: token });
}
